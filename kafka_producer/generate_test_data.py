...
session_telemetry = pd.DataFrame(columns=telemetry_column_types.keys()).astype(
    telemetry_column_types
)
for driver in drivers:
    driver_laps = laps[laps["Driver"] == driver]["LapNumber"].unique().tolist()
    for lap in driver_laps[3:5]:
        ...
        session_telemetry = pd.concat(
            [session_telemetry, final_tele_laps_data], axis=0, ignore_index=True
        )

# session_laps.sort_values(by = "LapNumber", inplace = True)

session_telemetry.sort_values(by="Date", inplace=True)
session_telemetry["date_delta"] = (
    session_telemetry["Date"].diff().fillna(pd.Timedelta(seconds=0)).dt.total_seconds()
    * 1000
)
