...
def acked(err, msg):
    if err is not None:
        ...
        print(
            f"Message sent to partition {msg.partition()} (Driver ID: {msg.key().decode('utf-8')}) Topic: {msg.topic()}"
        )
...
with open("tele_data.jsonl", "r", encoding="utf-8") as file:
    for row in file:
        ...
        send_telemetry(message=data, producer=producer, topic="f1_tele", key="Driver")
