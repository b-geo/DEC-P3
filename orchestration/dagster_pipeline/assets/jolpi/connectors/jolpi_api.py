...
class JolpiAPI:
    ...
    def _get_current_round(self) -> int:
        ...
        past_dates_df = events_df[
            events_df["date"].dt.date < self.today_date
        ]  # i only want completed rounds
        ...
        if not past_dates_df.empty:
            ...
            print(
                "No valid dates <= today found for this round. Returning 1."
            )  # logger warning
            return 1
    ...
    def _get_events(self) -> pd.DataFrame:
        ...
        events_resp = requests.get(
            f"{self.base_url}{self.today_year}/races/{self.params}", timeout=10
        )
    ...
    def _get_circuits(self) -> pd.DataFrame:
        # circuits
        circuits_resp = requests.get(
            f"{self.base_url}circuits/{self.params}", timeout=10
        )
    ...
    def _get_constructor_standings(self) -> pd.DataFrame:
        ...
        cons_resp = requests.get(
            f"{self.base_url}{self.today_year}/{self.current_round}/constructorstandings/{self.params}",
            timeout=10,
        )
        cons_resp.raise_for_status()
        cons_json = cons_resp.json()["MRData"]["StandingsTable"]["StandingsLists"][0][
            "ConstructorStandings"
        ]
        cons_df = pd.DataFrame(cons_json)
        cols_for_json = ["Constructor"]
    ...
    def _get_driver_standings(self) -> pd.DataFrame:
        ...
        drivers_resp = requests.get(
            f"{self.base_url}{self.today_year}/{self.current_round}/driverstandings/{self.params}",
            timeout=10,
        )
        drivers_resp.raise_for_status()
        drivers_json = drivers_resp.json()["MRData"]["StandingsTable"][
            "StandingsLists"
        ][0]["DriverStandings"]
        drivers_df = pd.DataFrame(drivers_json)
        cols_for_json = ["Driver", "Constructors"]
